...
public class AlumnoCreate extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		AlumnoPOJO al = new AlumnoPOJO(request.getParameter("nombre").trim().toUpperCase(),
				request.getParameter("apellido").trim().toUpperCase());

		AlumnoDAO al_dao = new AlumnoDAO(this.conn);
		if (al_dao.create(al)) { // true si exito
			...
			try (PrintWriter out = response.getWriter()) {
				...
				va.error("Error al dar de alta el alumno",
						"No ha sido posible dar de alta el alumno. Intentelo de nuevo pasados unos minutos. Si el error persiste, contacte con el servicio técnico.");
				va.pintaPie();
			}
		}
	}
