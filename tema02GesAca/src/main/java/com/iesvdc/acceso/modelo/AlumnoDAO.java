...
public class AlumnoDAO {
	...
	public AlumnoPOJO findById(Integer id) {
		...
		try {
			...
			rs.next();
			al = new AlumnoPOJO(rs.getInt("id"), rs.getString("nombre"), rs.getString("apellido"));
			// conn.close();
		} catch (SQLException ex) {
			al = null;
		}
	 ...
	 * @return Devuelve: null: si hay algún error (no se puede conectar a la
	 *         BBDD...). <br>
	 ...
	 * 
	 * @param apellido El nombre a buscar
	 * @return Devuelve: null: si hay algún error (no se puede conectar a la
	 *         BBDD...). <br>
	 *         ArrayList vacío (length == 0): si no hay nadie con ese nombre. <br>
	 *         ArrayList con Alumnos: si hay alumnos con ese nombre.<br>
	 ...
	 * 
	 * @param old_al El objeto que contiene los datos antiguos del alumno
	 * @param new_al El objeto que contiene los datos nuevos del alumno
	 * @return true si se lleva a cabo correctamente <br>
	 *         false si no se actualiza nada (error de conexión, no estaba el alumno
	 *         en la BBDD...) <br>
	 */
	 ...
	 * Este método actualiza un alumno en la BBDD
	 * 
	 * @param old_id El id antiguo del alumno
	 * @param new_al El objeto que contiene al alumno actualizado
	 * @return true si se lleva a cabo correctamente <br>
	 *         false si no se actualiza nada (error de conexión, no estaba el alumno
	 *         en la BBDD...) <br>
	 */
	public boolean update(Integer old_id, AlumnoPOJO new_al) {
		...
		try {
			// Conexion conexion = new Conexion();
			// Connection conn = conexion.getConnection();
			String sql = "UPDATE ALUMNO SET id=?, nombre=?, apellido=? WHERE id=?";
			PreparedStatement pstmt = conn.prepareStatement(sql);
	 ...
	 * @return true si borra un alumno <br>
